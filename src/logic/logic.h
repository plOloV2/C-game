...
struct gameState
{
    ...
    float mouseX;
    float mouseY;
    bool mouseButtonLeftFree;
    bool mouseButtonRightFree;
};

struct gameState* initGameState(){
    struct gameState* result = (struct gameState*) malloc(sizeof(uint8_t) + sizeof(uint16_t) + sizeof(float)*2);
    if(result == NULL)
        return NULL;
...
void eventHandler(struct gameState* game, SDL_Event event){
    ...
    game->mouseButtonLeftFree = true;
    game->mouseButtonRightFree = true;
    ...
    while(SDL_PollEvent(&event)){                               //loops untill all events are processed
        ...
        switch(event.type){
            ...
            case SDL_EVENT_MOUSE_BUTTON_DOWN:                   //mouse button press detection
                switch(event.button.button){
                    case SDL_BUTTON_LEFT:
                        ...
                        game->mouseButtonLeftFree = false;
                        break;

                    case SDL_BUTTON_RIGHT:
                        game->keysPressed |= (1 << MOUSE_KEY_PRESS_RIGHT);
                        game->mouseButtonRightFree = false;
                        break;
                }
            ...
            case SDL_EVENT_MOUSE_BUTTON_UP:                     //mouse button release detection
                switch(event.button.button){
                    case SDL_BUTTON_LEFT:
                        if(game->mouseButtonLeftFree)
                            game->keysPressed &= ~(1 << MOUSE_KEY_PRESS_LEFT);
                        break;

                    case SDL_BUTTON_RIGHT:
                        if(game->mouseButtonRightFree)
                            game->keysPressed &= ~(1 << MOUSE_KEY_PRESS_RIGHT);
                        break;
                }
